 ...
 * @return FLASH_OK if the read is successful, @ref error_codes otherwise.
 */
w25q_error_codes_t w25q128jw_read_standard_dma(uint32_t addr, void *data, uint32_t length, uint8_t no_wait_init_dma,
                                               uint8_t no_sanity_checks);
...
/**
 * @brief Wait for w25q128jw_read_quad_dma_async (works only used along with that function) and take care of the last
 * bytes, if present
 *
 * @param data pointer to the data buffer.
 * @param length number of bytes to write.

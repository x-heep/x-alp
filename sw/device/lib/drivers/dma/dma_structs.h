...
#define dma_peri(channel) ((volatile dma *) DMA_BASE_ADDRESS)
...
typedef struct {
  ...
  uint32_t HW_CONFIG_MODE;                        /*!< Hardware configuration mode*/

  uint32_t SLOT_WAIT_COUNTER;                     /*!< A counter to wait before submitting the next req when using slots*/

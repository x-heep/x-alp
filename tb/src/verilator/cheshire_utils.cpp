...
unsigned int CheshireUtils::extract_memory_type() {
  ...
  if (memory_type != "dram" && memory_type != "spm") {
      std::cerr << "[ERROR]: Unsupported memory type '" << memory_type << "'. Supported types are 'dram' and 'spm'." << std::endl;
      return EXIT_FAILURE;
...
unsigned long long CheshireUtils::get_max_sim_time(bool& run_all)
{
  ...
  if(arg_max_sim_time.empty()){
    ...
    if(u == arg_max_sim_time.length())  max_sim_time *= CLK_PERIOD_ps; // no suffix: clock cycles
      ...
      std::cout<<"[TESTBENCH]: ERROR: Unsupported suffix '"<<arg_max_sim_time.substr(u)<<"' for +MAX_CYCLES="<<std::endl;
      exit(EXIT_FAILURE);
    }
...
std::string CheshireUtils::get_boot_mode()
{
  ...
  if (arg_boot_mode.empty()) {
    ...
    if (arg_boot_mode == "sd") {
      ...
      std::cerr << "[TESTBENCH]: Wrong Boot Option specified, supported options are ( force, jtag)" << std::endl;

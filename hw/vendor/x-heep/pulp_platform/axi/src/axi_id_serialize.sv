...
module axi_id_serialize #(
    ...
    parameter int unsigned IdMap[axi_pkg::iomsb(
IdMapNumEntries
):0][0:1] = '{default: {32'b0, 32'b0}}
) (
    ...
    for (genvar i = 0; i < AxiMstPortMaxUniqIds; i++) begin : gen_serializers
        ...
        always_comb begin
            ...
            tmp_serializer_resps[i].b.id = {
                {AxiSlvPortIdWidth - 1{1'b0}}, from_serializer_resps[i].b.id
            };
            tmp_serializer_resps[i].r.id = {
                {AxiSlvPortIdWidth - 1{1'b0}}, from_serializer_resps[i].r.id
            };
        end
    end

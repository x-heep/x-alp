...
`ifndef COMMON_CELLS_ASSERTS_OFF
    initial begin : proc_check_parameters
        `ASSUME_I(addr_width_mismatch, $bits(addr_i) == $bits(addr_map_i[0].start_addr), $sformatf(
        ...
        if (!$isunknown(addr_map_i) && ~config_ongoing_i) begin
          ...
          addr_map_i[i].end_addr == '0,
              ...
              #####################################################",
                ...
                // check the SLV ids
                `ASSUME_I(check_idx, addr_map_i[i].idx < NoIndices, $sformatf(

...
module prim_fifo_async #(
    ...
    if (Depth > 2) begin : g_full_gray_conversion
        ...
        function automatic [PTR_WIDTH-1:0] dec2gray(input logic [PTR_WIDTH-1:0] decval);
            ...
            dec2gray = {
                decval[PTR_WIDTH-1], {1'b0, decval_in[PTR_WIDTH-2:1]} ^ decval_in[PTR_WIDTH-2:0]
            };
        endfunction
    ...
    `ASSERT(GrayWptr_A, $countones(fifo_wptr_gray_q ^ $past(fifo_wptr_gray_q)) <= 1, clk_wr_i,
            !rst_wr_ni)
    `ASSERT(GrayRptr_A, $countones(fifo_rptr_gray_q ^ $past(fifo_rptr_gray_q)) <= 1, clk_rd_i,
            !rst_rd_ni)

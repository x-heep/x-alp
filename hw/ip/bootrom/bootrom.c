...
int boot_passive(uint64_t core_freq) {
  volatile uint32_t main_address = *reg32(SOC_CTRL_BASE_ADDRESS, SOC_CTRL_BOOT_ADDRESS_REG_OFFSET);

  while (*reg32(SOC_CTRL_BASE_ADDRESS, SOC_CTRL_BOOT_EXIT_LOOP_REG_OFFSET) == 0);

...
{ name: "soc_ctrl"
  ...
  registers: [
    ...
    { name:     "BOOT_SELECT"
      desc:     "Boot Select Value - Used to decide boot mode"
      swaccess: "ro"
      hwaccess: "hrw"
      fields: [
        { bits: "1:0", name: "BOOT_SELECT", desc: "Boot Select Reg", 
          enum: [
            { name: "BOOT_SELECT_PASSIVE",   value: "0", desc: "Passive Boot (wait for JTAG or UART commands)" }
          ]
        }
      ]
    }
